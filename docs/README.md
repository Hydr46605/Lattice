...
For shared-runtime mode, install the standalone `Lattice` plugin on the server, compile dependent plugins with `compileOnly("io.github.hydr46605:lattice-paper:0.8.6")`, and declare a hard Paper dependency with `join-classpath: true`.

```kotlin
dependencies {
    compileOnly("io.github.hydr46605:lattice-paper:0.8.6")
}
```
...
```kotlin
dependencies {
    implementation("io.github.hydr46605:lattice-core:0.8.6")
}
```
...
For legacy isolated mode, shade `io.github.hydr46605:lattice-paper:0.8.6` into the dependent plugin and do not expect shared pools or aggregate diagnostics.
